...
=head1 CONSTRUCTOR
  ...
  $OBJ = WWW::AUR->new( basepath => $BASEPATH || '/tmp/WWW-AUR',
                        dlpath   => $DLPATH   || "$BASEPATH/src",
                        extpath  => $EXTPATH  || "$BASEPATH/build",
                        destpath => $DESTPATH || "$BASEPATH/cache" );
...
=item C<$DESTPATH> (optional)

A directory to store built binary package files. Binary package files
are built from the source package files by using I<makepkg>.
...
object that is created. Package objects are created when methods such
as L</METHODS/search> and L</METHODS/find>.
...
L</METHODS/maintainer>, the L<WWW::AUR::Login> object created by
L</METHODS/login>, and the L<WWW::AUR::Iterator> object created by
L</METHODS/packages>.
